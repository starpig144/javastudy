...
//	equals 메소드
//	비교하는 문자열이 등일하면 true, 아니면 false 반환
...
//	2. substriing(int begin, int end): 인덱스 begin 부터 end 이전까지 반환(begin <=추출범위 <end)
...
//	2. indexOf(Stiring str , int index) : 인덱스 index부터 str을 검색
	...
	public static void lastIdexOf() {
//	lastIdexOf
//	발견된 마지막 문자열의 인덱스를 반환
//	나머지 특성은 indexOf 와 동일
		String slogan = "걱정한다고 걱정이 없어지면 걱정이 없겠네";
		int idx1 = slogan.lastIndexOf("걱정");
		
		int idx2 = slogan.lastIndexOf("구자영");
...
//	matches
//	문자열이 지정된 정규식 패턴(Regular Expression)으로 포함 하면 true 반환
		
	String name = "구자영";
	
	if(name.startsWith("구")) {
		System.out.println("구씨다");
		
	}else {
		System.out.println("구씨가 아니다");
	}
	}
...
//	public class String implements CharSequence{}
	...
	String email ="starpig14@naver.com";
...
//		출력 폭 지정하기
		System.out.println(String.format("%10d", number)); // %10d : 10 자리로 표시하시오 
		System.out.println(String.format("%-10d", number)); // %-10d : 10 자리로 표시하시오 
